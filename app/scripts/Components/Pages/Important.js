...
export default React.createClass({
  ...
  updateTasks() {
    let tasks = _.where(store.importantTasks.toJSON(), {listId: this.props.params.id});
    this.setState({tasks: store.importantTasks.toJSON()})
  },
  showOptions() {
  ...
  componentDidMount() {
    store.importantTasks.on('update', this.updateTasks);
    store.importantTasks.getTasks();
  },
  componentWillUnmount() {
    store.importantTasks.off('update', this.updateTasks);
  },
  render() {
